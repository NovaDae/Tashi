...
const grid = document.getElementById('projectsGrid');
    ...
    grid.innerHTML = projects.map(project =>      `<div class="project-card" onclick="openModal(${project.id})">          <img src="${project.thumbnail}" alt="${project.title}" class="project-image">          <div class="project-overlay">              <div class="project-category">${project.category}</div>              <h3 class="project-title">${project.title}</h3>          </div>      </div>` ).join('');
    }
...
const project = projects.find(p => [p.id](http://p.id/) === projectId);
...
const index = projects.findIndex(p => [p.id](http://p.id/) === projectId);
if (index !== -1) {
...
return projects.find(p => [p.id](http://p.id/) === projectId) || null;
}
